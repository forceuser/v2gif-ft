...
globby.sync(["../src/sprites/**/*.svg"], {cwd: __dirname}).forEach(fn => {
	...
	content.replace(/ id=['"](.*?)['"]/gim, (all, p1) => {
		idLast++;
		if (p1 !== id) {
			content = content.replace(
				new RegExp(`id="${p1}"`, "igm"),
				`id="ugid${idLast}"`
			);
			content = content.replace(
				new RegExp(`#${p1}`, "igm"),
...
Object.keys(categories).forEach(category => {
	...
	try {
		...
		const tags = ["linearGradient", "radialGradient", "mask", "clipPath"];
		let nodes = [];
										...
										<div class="item" onclick="copy('${category}#${icon.id}')">
											<div class="item-icon-container">
												<svg class="svg-icon" viewBox="${icon.viewBox}">
													<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#${category}__${
	icon.id
}"></use>
												</svg>
											</div>
